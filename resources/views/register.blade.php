    ...
    <div class="flex items-center justify-center h-screen bg-yellow-200">
        <div class="bg-white rounded-3xl border shadow-xl p-8 w-3/6">
            <div class="flex justify-between items-center mb-4">
                ...
                <div>
                    <span class="font-bold text-green-500">+ ${{ $currencys->price  }} (3%)</span><br />
                    <span class="font-medium text-xs text-gray-500 flex justify-end">0.382 coin</span>
                </div>
            </div>
            <div>
                <h3 class="font-semibold text-sm text-gray-400"> {{ $currencys->name }}</h3>
                <h1 class="font-semibold text-xl text-gray-700">${{ $currencys->price  }}</h1>
                <h1 class="font-semibold text-xl text-gray-700">{{ $currencys->code  }}</h1>
            </div>
        </div>
    </div>
    @endforeach
